...
/* from httpd-2.2.31/srclib/apr-util/include/apr_base64.h */
/* FIXME - check validity for binary byte string */
 ...
 */ 
int apr_base64_decode(unsigned char *plain_dst, const char *coded_src);
 ...
 * @param plain_src The original string in plain text
 * @param len_plain_src The length of the plain text string
 * @return the encoded string which should be freed by caller
 */
char *base64encode(char *plain_src, int len_plain_src);

/**
 * Decode a string to plain text
 * @param coded_src The encoded string
 * @return the decoded string which should be freed by caller
 */ 
char *base64decode(const char *coded_src);
