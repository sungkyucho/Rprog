...
set_secret(TSS_HCONTEXT hContext, TSS_HOBJECT hObj, TSS_HPOLICY *hPolicy)
{
	...
	result = Tspi_Policy_SetSecret(hLocalPolicy, 
			//secret_mode, secret_len, secret);
...
//			TESTSUITE_OWNER_SECRET_MODE, TESTSUITE_OWNER_SECRET_LEN, TESTSUITE_OWNER_SECRET);
				  TSS_SECRET_MODE_SHA1,
				  20,
				  well_known_secret);
	if ( result != TSS_SUCCESS ) {
...
//				TESTSUITE_OWNER_SECRET_MODE, TESTSUITE_OWNER_SECRET_LEN, TESTSUITE_OWNER_SECRET);
	...
	result = Tspi_Policy_SetSecret(hPolicy, 
//			TESTSUITE_KEY_SECRET_MODE, TESTSUITE_KEY_SECRET_LEN, TESTSUITE_KEY_SECRET);
				  TSS_SECRET_MODE_SHA1,
				  20,
				  well_known_secret);
	if ( result != TSS_SUCCESS )
